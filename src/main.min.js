document.addEventListener('click', e => {
    ...
    if (e.target.closest('.js-gallery__item')) {
        ...
        const gallerySliderThumbsString = `<div class="js-gallery-slider-thumbs">
                                            ...
                                            <div class="swiper-container">
                                                <div class="swiper-wrapper">
                                                    ${thumbs.map(thumb => `<div class="swiper-slide"><div class="ratio ${thumb.type === 'video' ? 'video-overlay' : ''}"><img src="${thumb.thumb}" alt=""></div></div>`).join('')}
                                                </div>
                                            </div>
        ...
        const galleryInit = () => {
            ...
            const sliderThumbsContainer = sliderThumbs.querySelector('.swiper-container');
            const sliderThumbsBtnPrev = sliderThumbs.querySelector('.swiper-button-prev');
            const sliderThumbsBtnNext = sliderThumbs.querySelector('.swiper-button-next');
    ...
    if (e.target.closest('.js-select .js-select__control')) {
        ...
        const dropdown = control.closest('.js-select').querySelector('.js-select__dropdown');
        if (dropdown) {
    ...
    if (e.target.closest('.js-select .js-select__dropdown li')) {
        ...
        const control = liSelected.closest('.js-select').querySelector('.js-select__control');
        const dropdown = liSelected.closest('.js-select').querySelector('.js-select__dropdown');
    ...
    if (e.target.closest('.js-toggle-btn')) {
        ...
        document.querySelectorAll('.js-toggle-btn.active, .js-toggle-collapse.active').forEach(el => el.classList.remove('active'));
        collapse.classList.toggle('active', !isShowing);
        btn.classList.toggle('active', !isShowing);
    } else if (e.target.closest('.js-close-btn') || !e.target.closest('.js-toggle-collapse')) {
        document.querySelectorAll('.js-toggle-btn.active, .js-toggle-collapse.active').forEach(el => el.classList.remove('active'));
    }
    ...
    if (e.target.closest('.order-item__bottom__btn')) {
        ...
        const collapseEl = btn.closest('.order-item').querySelector('.order-item__collapse');
    ...
    if (e.target.closest('.add-to-cart-modal .add-to-cart-modal-btn')) {
        ...
        const {top: cartIconTop, left: cartIconLeft, width: cartIconWidth} = hdCartIcon.getBoundingClientRect();
